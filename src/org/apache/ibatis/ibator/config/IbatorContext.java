...
public class IbatorContext extends PropertyHolder {
	...
	public XmlElement toXmlElement() {
		...
		if (controllerGeneratorConfiguration != null) {
			xmlElement.addElement(controllerGeneratorConfiguration.toXmlElement());
		}

		if (htmlGeneratorConfiguration != null) {
			xmlElement.addElement(htmlGeneratorConfiguration.toXmlElement());
		}
	...
	public void introspectTables(ProgressCallback callback, List<String> warnings, Set<String> fullyQualifiedTableNames)
		...
		try {
			...
			DatabaseIntrospector databaseIntrospector = new DatabaseIntrospector(IbatorContext.this,
					connection.getMetaData(), javaTypeResolver, warnings);

			for (TableConfiguration tc : tableConfigurations) {
				String tableName = StringUtility.composeFullyQualifiedTableName(tc.getCatalog(), tc.getSchema(),
						tc.getTableName(), '.');
	...
	public void generateFiles(ProgressCallback callback, List<GeneratedJavaFile> generatedJavaFiles,
			List<GeneratedXmlFile> generatedXmlFiles, List<GeneratedHtmlFile> generatedHtmlFiles,List<String> warnings) throws InterruptedException {
		...
		if (introspectedTables != null) {
			for (IntrospectedTable introspectedTable : introspectedTables) {
				...
				generatedHtmlFiles.addAll(introspectedTable.getGeneratedHtmlFiles(introspectedTable));
