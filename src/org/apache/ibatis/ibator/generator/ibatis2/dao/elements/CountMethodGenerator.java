...
public class CountMethodGenerator extends AbstractDAOElementGenerator {
    ...
    public void addImplementationElements(TopLevelClass topLevelClass) {
         ...
         sb.append(daoTemplate.getQueryForObjectMethod(introspectedTable.getBaseRecordType().getFullyQualifiedName(), getDAOMethodNameCalculator()
                 .getCountMethodName(introspectedTable), paramBaseRecord())); //$NON-NLS-1$
         method.addBodyLine(sb.toString());
         method.addBodyLine("return count.intValue();"); //$NON-NLS-1$

         if (ibatorContext.getPlugins().daoCountGenerated(method, topLevelClass, introspectedTable)) {
    ...
    public void addInterfaceElements(Interface interfaze) {
        ...
        if (ibatorContext.getPlugins().daoCountGenerated(method, interfaze, introspectedTable)) {
    ...
    private Method getMethodShell(Set<FullyQualifiedJavaType> importedTypes) {
        ...
        method.setName(getDAOMethodNameCalculator()
                .getCountMethodName(introspectedTable));
